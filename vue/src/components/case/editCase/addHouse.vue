                ...
                <div class="imgsList mt20">
                    <el-row :gutter="10">
                        <draggable v-model="d.list" :options="dragOptions">
                            <el-col class="ms_full_w caseitem" :class="{'el-small':layoutMini}" v-for='(x,index) in d.list' style='margin-bottom:10px;overflow:hidden;position:relative;width:300px;' :key='x.full_path_url' v-if="!x.delflag">
                                <el-card :body-style="{ padding: '0px' }" :class="{active:selectList[index]}">
                                    <div style="padding:10px;" class="flex-box flex-between" v-show="!layoutMini">
                                        ...
                                        <a :href="x.full_path_url" target="_blank" class="a-hover f14">
                                            <i class="el-icon-download"></i>原图</a>
                                    </div>
                                    <div class="cover-body handle" style='cursor:move;padding-bottom:75%;'>
                                        <!-- <img :src="x.full_path_url" alt="" class="cover"> -->
                                        <img v-lazy='x.full_path_url + "!zmm"' alt="" class="cover">
                                    </div>
                                    <div style="padding: 10px;overflow:hidden;" v-show="!layoutMini">
                                        <el-form-item :prop="'list.' + index + '.desc'" :rules="{required: true, message: '描述不能为空', trigger: 'blur,change'}">
                                            <el-input type="textarea" :rows="6" placeholder="请输入内容" v-model="x.desc" class="no">
                                            </el-input>
                                        </el-form-item>
                                    </div>
                                </el-card>
        ...
        methods: {
            ...
            getDeital() {
                _self.$id({
                    msg: '请传入案例ID',
                    cb: function () {
                        _self.$http.get('case/housemap/list', {
                            params: {
                                id: _self.id
                            }
                        }).then(res => {
            ...
            submitFormFns(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        _self.$http.post('case/housemap/update', {
                            id: _self.id,
                            house_map: _self.d.list
                        }).then(res => {
                            if (res.codeFlag) {
                                _self.$message({
                                    type: 'success',
